...
ipcMain.handle("findAll", async () => {
  return await new VehicleRepository().findAll();
});

ipcMain.handle("findById", async (_: any, id: string) => {
  return await new VehicleRepository().findById(id);
});
...
ipcMain.handle("createUser", async (_: any, user: any) => {
  const { nome, email, password, data_nascimento } = user;
  const passwordHash = await hash(password, 12);
  const newUser = new User(nome, email, passwordHash, data_nascimento);
